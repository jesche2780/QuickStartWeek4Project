...
public class Week4Project { // Normal Java headers
...
int totalLetters = 0; // Started empty integer variable counter.
        for (String name : names) {
            totalLetters += name.length(); // Using an enhanced for loop to create a string variable "name" with each iteration within the "names" array, for each iteration, totalLetters is added to for each of the names array locations, which were allocated to a "name" variable.
        }
...
// int last = ages[ages.length - 1]; // The last number of an array is accessed by the "-1" of the array's length to location the last item in the array's contents.

//4. How do you access the first element of any array?
// int first = ages[0]; // The first element of an array is location "0"
...
public class StringRepeater { 
    ...
    public static void secondRepeatWord(String[] args) { // Second part of the repeater method is declared, with the String "result" created, where repeatWord is is added to the method "n" number of times and printed.
...
public class AverageComparator { 
    ...
    private static double average(double[] arr) { // This starts the method passing in arr that then calculates the averages of double "a" and double "b" to pass into the above method.
        double sum = 0;
        for (double v : arr) {
            sum += v;
        }
